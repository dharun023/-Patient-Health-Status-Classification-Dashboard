...
with colA:
    if "test_group1" not in df_filtered.columns:
        st.warning(
            "The `test_group1` column is not available in the filtered dataset."
        )
    else:
        tg = df_filtered[
            ["test_group1", "target", "test_name"]
        ].copy()
        ...
        # Remove Rh Typing records
        tg = tg[
            tg["test_name_clean"] != "rh typing"
        ].copy()
        ...
        # Remove empty test-group values
        tg = tg[
            tg["test_group1"].notna()
            & (tg["test_group1"] != "")
        ].copy()

        if tg.empty:
            st.info(
                "No valid test-group data is available after the current filters."
            )
        else:
            tg_stats = (
            ...
            fig_tg.update_layout(
                ...
                xaxis_range=[
                    ...
                    min(
                        110,
                        max(100, top_tg["abnormal_rate"].max() + 10),
                    ),
                ],
            )
